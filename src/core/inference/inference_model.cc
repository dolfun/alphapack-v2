...
auto InferenceModel::infer(const InferenceInfo& info) const -> void {
  ...
  const auto gpu_options = torch::TensorOptions{}.dtype(torch::kFloat32).device(torch::kCUDA);

  // Copy input to CPU
  const auto image_input_cpu =
    torch::from_blob(info.image_input.data(), info.image_input_shape, cpu_options).permute({0, 3, 1, 2});
  ...
  const auto additional_input_cpu =
    torch::from_blob(info.additional_input.data(), info.additional_input_shape, cpu_options);
  ...
  // Copy output to CPU
  const auto policy_output_cpu = torch::from_blob(info.policy_output.data(), info.policy_output_shape, cpu_options);
  (void)policy_output_cpu.copy_(policy_output_gpu, true);

  const auto value_output_cpu = torch::from_blob(info.value_output.data(), info.value_output_shape, cpu_options);
