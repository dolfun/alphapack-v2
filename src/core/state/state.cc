...
template <typename T, std::size_t BoxN, std::size_t BoxM>
constexpr auto get_2d_window_max_in_box(
  const NdArray<T, BoxN, BoxM>& box,
  std::size_t window_n,
  std::size_t window_m
) {
  NdArray<T, BoxN, BoxM> result{};

  for (std::size_t x = 0; x < BoxN; ++x) {
    for (std::size_t y = 0; y <= BoxM - window_m; ++y) {
  ...
  for (std::size_t y = 0; y <= BoxM - window_m; ++y) {
    for (std::size_t x = 0; x <= BoxN - window_n; ++x) {
...
auto State::feasible_actions() const -> std::vector<Action> {
  if (m_items.front().placed) {
    return {};
  }
...
auto State::update_feasibility_info_with_front_item() noexcept -> void {
  ...
  for (std::size_t x = 0; x <= FeasibilityInfo::shape[0] - front_item.shape.x; ++x) {
    for (std::size_t y = 0; y <= FeasibilityInfo::shape[1] - front_item.shape.y; ++y) {
      ...
      if (static_cast<std::uint32_t>(max_height + front_item.shape.z) <= bin_height) {
